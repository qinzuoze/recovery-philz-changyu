...
void handle_failure() {
    ...
    ui_print("/tmp/recovery.log 复制到 /sdcard/clockworkmod/philz_recovery.log\n");
    ui_print("发送文件到 Phil3759 @xda\n");
}
...
void wipe_data_menu() {
    const char* headers[] = { "选择擦拭选项", NULL };
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 1: {
                if (confirm_selection("清空 cache 分区 ?", "是 - 清空 cache")) {
                    ui_print("\n-- 清空 cache中...\n");
            ...
            case 2: {
                ...
                if (confirm_selection("清空 dalvik cache ?", "是 - 清空 dalvik cache")) {
                    ui_print("\n-- 清空 dalvik cache中...\n");
                    __system("rm -r /data/dalvik-cache");
                    __system("rm -r /cache/dalvik-cache");
                    __system("rm -r /sd-ext/dalvik-cache");
                    ui_print("清空Dalvik Cache完成.\n");
                }
                break;
            }
            case 3: {
                ...
                if (confirm_with_headers(headers, "是 - 清空用户和系统数据")) {
                    wipe_data(false);
                    ui_print("-- 清空 system中...\n");
                    erase_volume("/system");
                    if (volume_for_path("/preload") != NULL) {
                        ui_print("-- 清空 preload中...\n");
                        erase_volume("/preload");
                    }
                    ui_print("现在刷入一个新的ROM.\n");
                }
                break;
            }
            case 4: {
                ...
                if (confirm_with_headers(headers, "是 - 清空所有用户媒体数据")) {
                    ui_print("\n-- 清空用户媒体数据中...\n");
...
void show_multi_flash_menu() {
    const char* headers_dir[] = { "选择一个ZIP包", NULL };
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        if (no_files_found) {
            ui_print("zip文件至少必须在一个子文件夹里 %s\n", tmp);
            ui_print("看看其他存储设备...\n");
        }
    } else {
        LOGI("%s 没有文件，搜索其他存储设备...\n", tmp);
    }
    ...
    if (numFiles == 0) {
        ...
        if (chosen_item == 1) {
            ...
            sprintf(confirm, "是 - 从 %s 安装", BaseName(zip_folder));
        ...
        while (fgets(script_line, SCRIPT_COMMAND_SIZE, fp) != NULL && ret_val == 0) {
            ...
            if (strcmp(command, "install") == 0) {
                ...
                if (strcmp(value, "cache") == 0 || strcmp(value, "/cache") == 0) {
                    ui_print("-- 清空 Cache 分区...\n");
                    erase_volume("/cache");
                    ui_print("-- 清空Cache分区完成!\n");
                } else if (strcmp(value, "dalvik") == 0 || strcmp(value, "dalvick") == 0 || strcmp(value, "dalvikcache") == 0 || strcmp(value, "dalvickcache") == 0) {
                    ui_print("-- 清空 Dalvik Cache...\n");
                    if (0 != ensure_path_mounted("/data")) {
                        ret_val = 1;
                        break;
                    }
...
void get_rom_name(char *rom_name) {
    ...
    while ((key = rom_id_key[i]) != NULL && strcmp(rom_name, "noname") == 0) {
        if (read_config_file("/system/build.prop", key, rom_name, "noname") < 0) {
            ui_print("打开 /system/build.prop 失败!\n");
...
void misc_nandroid_menu() {
    ...
    int fmt;
    for (;;) {
        if (enable_md5sum.value) ui_format_gui_menu(item_md5, "检验MD5值", "(x)");
        else ui_format_gui_menu(item_md5, "检验MD5值", "( )");
        ...
        if (show_nandroid_size_progress.value)
            ui_format_gui_menu(item_size_progress, "显示Nandroid大小的进度", "(x)");
        else ui_format_gui_menu(item_size_progress, "显示Nandroid大小的进度", "( )");
        list[3] = item_size_progress;

        if (use_nandroid_simple_logging.value)
            ui_format_gui_menu(item_use_nandroid_simple_logging, "使用简单的log（快）", "(x)");
        else ui_format_gui_menu(item_use_nandroid_simple_logging, "使用简单的log（快）", "( )");
        ...
        if (fmt == NANDROID_BACKUP_FORMAT_TGZ) {
            if (compression_value.value == TAR_GZ_FAST)
                ...
                ui_format_gui_menu(item_compress, "压缩", "普通");
            ...
            ui_format_gui_menu(item_compress, "压缩", "无");
...
int show_custom_zip_menu() {
    const char* headers[] = { "选中一个刷机包", NULL };
...
*/
    ...
    if (chosen_item !=  GO_BACK) {
        ...
        confirm = confirm_selection("安装选定的文件?", tmp);
...
void show_twrp_restore_menu(const char* backup_volume) {
    ...
    const char* headers[] = { "选择一个备份的文件来还原", NULL };
    ...
    char backup_source[PATH_MAX];
    sprintf(backup_source, "%s", DirName(file));
    ui_print("%s 将恢复到选定的分区!\n", backup_source);
    sprintf(confirm, "是 - 恢复 %s", BaseName(backup_source));
    if (confirm_selection("从这个备份还原 ?", confirm))
...
static void custom_restore_handler(const char* backup_volume, const char* backup_folder) {
    ...
    char backup_source[PATH_MAX];
    char* file = NULL;
    char* confirm_install = "从这个备份还原?";
    const char* headers[] = { "选择一个备份来还原", NULL };
    ...
    if (backup_efs == RESTORE_EFS_IMG) {
        ...
        if (vol != NULL) {
            ui_print("%s 将刷写 /modem!\n", backup_source);
            sprintf(tmp, "是 - 恢复 %s", backup_source);
            if (confirm_selection(confirm_install, tmp))
                dd_raw_restore_handler(file, "/modem");
        } else
            LOGE("没有 /modem 分区文件进行刷写\n");
        ...
        if (vol != NULL) {
            ui_print("%s 将刷写 /radio!\n", backup_source);
            sprintf(tmp, "是 - 恢复 %s", backup_source);
            if (confirm_selection(confirm_install, tmp))
                dd_raw_restore_handler(file, "/radio");
        } else
            LOGE("没有 /radio 分区文件进行刷写\n");
    } else {
        // process restore job
...
void show_philz_settings_menu()
{
    ...
    for (;;) {
        if (check_root_and_recovery.value)
            ui_format_gui_menu(item_check_root_and_recovery, "退出时验证Root", "(x)");
        else ui_format_gui_menu(item_check_root_and_recovery, "退出时验证Root", "( )");

        if (auto_restore_settings.value)
            ui_format_gui_menu(item_auto_restore, "自动还原设置", "(x)");
        else ui_format_gui_menu(item_auto_restore, "自动还原设置", "( )");
        ...
        switch (chosen_item) {
            ...
            case 3: {
                if (confirm_selection("重置所有Recovery设置?", "是 - 重置回默认")) {
                    char settings_copy[PATH_MAX];
                    sprintf(settings_copy, "%s/%s", get_primary_storage_path(), PHILZ_SETTINGS_FILE2);
...
int show_install_update_menu() {
    ...
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes]     = "从根目录选择刷机包";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 1] = "从最后一次打开的目录选择刷机包";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 2] = "使用ADB从电脑推送刷机";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 3] = "安装多个 zip 文件";
    ...
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 6] = "设置默认的根目录文件夹";
    ...
    for (;;) {
        if (signature_check_enabled.value)
            ui_format_gui_menu(item_toggle_signature_check, "签名认证", "(x)");
        else ui_format_gui_menu(item_toggle_signature_check, "签名认证", "( )");

        if (install_zip_verify_md5.value)
            ui_format_gui_menu(item_install_zip_verify_md5, "检验zip的MD5值", "(x)");
        else ui_format_gui_menu(item_install_zip_verify_md5, "检验zip的MD5值", "( )");
...
void show_nandroid_restore_menu(const char* path) {
    ...
    if (confirm_selection("确认要还原?", "是 - 还原"))
...
void show_nandroid_delete_menu(const char* volume_path) {
    ...
    for(;;) {
        ...
        if (confirm_selection("确认要删除?", tmp)) {
...
void show_partition_format_menu() {
    ...
    for (;;) {
        ...
        if (!is_data_media()) {
...
#ifdef USE_F2FS
            list[formatable_volumes] = "转换 f2fs <-> ext4 ";
            ...
            list[formatable_volumes + 1] = NULL;
#ifdef USE_F2FS
            list[formatable_volumes + 1] = "转换 f2fs <-> ext4 ";
        ...
        if (is_data_media() && chosen_item == formatable_volumes) {
            ...
            ui_print("ext4 <-> f2fs 转换 %s\n", enable_f2fs_ext4_conversion ? "启用" : "禁用");
        }
#endif
    }
}
...
int show_partition_mounts_menu() {
    ...
    for (;;) {
        ...
        if (is_vold_ums_capable) {
            list[mountable_volumes] = "USB挂载存储设备";
...
void choose_default_backup_format() {
    const char* headers[] = { "默认备份的格式", "", NULL };
    ...
    switch (chosen_item) {
        case 0: {
            ...
            ui_print("设定默认备份的格式为tar.\n");
            break;
        }
        case 1: {
            write_string_to_file(path, "dup");
            ui_print("设定默认备份的格式为dedupe.\n");
            break;
        }
        case 2: {
            write_string_to_file(path, "tgz");
            ui_print("设定默认备份的格式为tar + gzip.\n");
            break;
        }
    }
}
...
void show_format_sdcard_menu(const char* path) {
    ...
    if (chosen_item < 0) // REFRESH or GO_BACK
        return;
    if (!confirm_selection("确认要格式化?", "是 - 格式化设备"))
        return;
